...
makeCacheMatrix <- function(x = matrix()) {
	...
	m <- NULL
	# This creates a function within this environment that creates a 
	# cached version of the matrix input y and stores it as matrix x in the new environment.
...
# This function will check to see if the getinv functions returns a non-null value (an inverse).
# If so, print a message and return the inverse.
# If not, call the setinv function within the list input
# to store the value of the inverse in the different environment.
cacheSolve <- function(x, ...) {
	...
	if(!is.null(m)) { # Check to see if the value of m is not null
			message("getting cached inverse") # Print a message to the console
			# that the inverse has been retrieved from the cache rather than re-calculated
			return(m) # End the function and return the value of m
	}
	# If the function makes it to this point, the value of m was NULL
	# and the inverse of x needs to be calculated.
...
# Test it out.
# A 1000x1000 matrix is randomly generated
# The functions are called that cache the matrix and its inverse
# system.time is called twice with cacheSolve
# The first call to cacheSolve needs to actually calculate the inverse
# There is a significant time difference in the second call because the cached inverse is retrieved
